...
# Ports that DimOS binds and that must be free before a new run.
_DIMOS_PORTS = (7779, 5555, 9877)


def _port_in_use(port: int) -> bool:
    """Return True if *port* is bound on localhost."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.settimeout(0.2)
        return s.connect_ex(("127.0.0.1", port)) == 0


def force_cleanup_all(timeout: float = 10.0) -> int:
    """Kill ALL orphaned DimOS worker processes and wait for ports to free.

    This is more aggressive than ``cleanup_stale`` — it kills every process
    whose command-line matches "dimos" (excluding the calling process) and
    then blocks until the well-known DimOS ports are released.
    ...
    # 1. Kill registered runs first (graceful → force).
    for entry in list_runs(alive_only=True):
        if entry.pid == my_pid:
            continue
        try:
    ...
    # 2. Kill any orphaned workers whose parent was already gone.
    #    Workers show up as python processes launched by the dimos CLI.
    #    Use pgrep with a narrow pattern to avoid hitting unrelated processes.
    try:
        result = subprocess.run(
            ["pgrep", "-f", "python.*dimos.robot.cli|python.*dimos.core.worker"],
            capture_output=True,
            text=True,
        )
        ...
        pass  # pgrep not available or bad output

    # 3. Wait for ports to release.
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        if not any(_port_in_use(p) for p in _DIMOS_PORTS):
            break
        time.sleep(0.5)
    else:
        busy = [p for p in _DIMOS_PORTS if _port_in_use(p)]
        if busy:
            logger.warning(f"Ports still in use after cleanup: {busy}")
