...
class UnitreeWebRTCConnection(Resource):
    def __init__(self, ip: str, mode: str = "ai") -> None:
        ...
        if self.ip.startswith("192.168.12."):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.warning(f"WebRTC connection attempt {attempt}/{max_retries} failed: {e}")
                if attempt < max_retries:
                    import time as _time
                    _time.sleep(5)
                else:
                    raise ConnectionError(f"Failed to connect to robot at {self.ip} after {max_retries} attempts") from e
    ...
    def connect(self) -> None:
        ...
        self.thread.start()
        self.connection_ready.wait(timeout=30)
