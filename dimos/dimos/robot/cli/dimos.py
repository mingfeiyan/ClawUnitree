...
def run(
    ...
    # Kill orphaned workers and wait for ports to free
    killed = force_cleanup_all()
    if killed:
        logger.info(f"Force-killed {killed} orphaned DimOS processes")

    # Port conflict check (after cleanup)
